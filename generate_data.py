...
class GenerateDataset:
    '''
    This Class Generates a Random 10,000 Images of size 28x28 of Visa and Mastercard, with Anonymous Names.
    '''
    def __init__(self):
        ...
        for i in progressbar.progressbar(range(10000)):
            if this_time:
                img = self.generate_img('Visa')
                card_type = 'Visa'
            if this_time == False:
                img = self.generate_img('MasterCard')
    ...
    def generate_img(self,type):
        try:
            standard_master = ['Gold','Platinum','Standard']
            standard = random.choice(standard_master)
            session = requests.session()
            burp0_url = f"https://herramientas-online.com:443/tarjvsmc.php?nombreth=Mr.+Donald+Duck&tj=4440542763922359&cl={standard}&mc={type}&co=IbboRH5F6VtNsccVpRwORcdzqc&code=6295"
